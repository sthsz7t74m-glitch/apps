(() => {
  ...
  function filtered(){
    ...
    return repo.all().filter(e=>{
      ...
      if(dayType&&!holidays.matches(e.date,dayType))return false;
      if(max&&(!e.priceMin||e.priceMin>max))return false;
      return true;
  ...
  function renderMain(){
    ...
    $("#netaCount").textContent=items.filter(ordinary).length;
    if(items[0]){const d=cards.formatDate(items[0].date);$("#nextDate").textContent=d.md;$("#nextDateSub").textContent=`${d.wd}曜 ${items[0].start}〜`;}
    else{$("#nextDate").textContent="—";$("#nextDateSub").textContent="該当なし";}
    $("#resultSummary").textContent=`${state.mode==="neta"?"ネタ中心":state.mode==="available"?"販売中のみ":"全公演"}：${items.length}件表示`;
  }

  function renderShows(){
    favorites.cleanupExpired(repo.all(),time);
    const items=repo.all().filter(e=>favorites.isShow(e)&&!time.isFavoriteExpired(e)).sort((a,b)=>`${a.date}${a.start}`.localeCompare(`${b.date}${b.start}`));
    renderList($("#favoriteShowsList"),items,"お気に入り公演はまだありません");
    $("#favoriteShowCount").textContent=items.length;
  }

  function renderArtists(){
    const names=[...favorites.artists].sort((a,b)=>a.localeCompare(b,"ja"));
    const el=$("#favoriteArtistsList");
    el.innerHTML=names.length?names.map(n=>`<article class="artist-favorite-card"><div><strong>★ ${D.esc(n)}</strong><span>今後の掲載公演 ${repo.all().filter(e=>e.performers.includes(n)&&time.endDate(e)>new Date()).length}件</span></div><button type="button" data-remove-artist="${D.esc(n)}">解除</button></article>`).join(""):`<div class="empty-state"><strong>お気に入り芸人はまだありません</strong></div>`;
    $("#favoriteArtistCount").textContent=names.length;
  }

  function renderRecommendations(){
    let items=repo.all().filter(e=>time.endDate(e)>new Date()).map(e=>recommendations.score(e)).filter(Boolean);
    if(state.recommendationFilter==="week")items=items.filter(i=>i.days<=7);
    if(state.recommendationFilter==="many")items=items.filter(i=>i.matched.length>=2);
    if(state.recommendationFilter==="value")items=items.filter(i=>i.event.priceMin&&i.event.priceMin<=2500);
    ...
    renderList($("#recommendationList"),items,"おすすめ公演はまだありません",i=>cards.render(i.event,{recommendation:i}));
  }

  function renderPerformers(){
    const el=$("#performerChips");
    el.innerHTML=repo.performers().map(n=>`<button class="performer-chip${state.performer===n?" is-active":""}" type="button" data-filter-artist="${D.esc(n)}">${D.esc(n)}</button>`).join("");
    $("#clearPerformer").hidden=!state.performer;
  }

  function updatePage(){
    ["list","recommendations","shows","artists"].forEach(p=>{
      const el=$(p==="list"?"#listPage":p==="recommendations"?"#recommendationsPage":p==="shows"?"#showsPage":"#artistsPage");
      if(el)el.hidden=state.page!==p;
    });
    document.querySelectorAll("[data-page]").forEach(b=>b.classList.toggle("is-active",b.dataset.page===state.page));
  }
  ...
  function init(){
    ...
    document.addEventListener("click",e=>{
      ...
      const sf=e.target.closest("[data-show-id]");if(sf){const event=repo.findById(sf.dataset.showId);if(event)preserve(()=>favorites.toggleShow(event));return;}
    ...
    $("#resetButton")?.addEventListener("click",()=>{
      ["#searchInput","#venueFilter","#genreFilter","#toDate","#statusFilter","#dayTypeFilter","#priceFilter"].forEach(s=>{if($(s))$(s).value="";});
      if($("#sortFilter"))$("#sortFilter").value="date";
      state.mode="neta";state.performer="";renderAll();
    });
    renderAll();
  }
