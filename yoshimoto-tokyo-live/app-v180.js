(() => {
  ...
  let favoriteShows = loadSet(SHOW_KEY);
  let favoriteArtists = loadSet(ARTIST_KEY);
  ...
  const elements = {
    ...
    list: $("#liveList"),
    favShowsList: $("#favoriteShowsList"),
    favArtistsList: $("#favoriteArtistsList"),
    search: $("#searchInput"),
    venue: $("#venueFilter"),
    genre: $("#genreFilter"),
    from: $("#fromDate"),
    ...
    artistCount: $("#favoriteArtistCount")
  };

  function saveFavorites() {
    localStorage.setItem(SHOW_KEY, JSON.stringify([...favoriteShows]));
    localStorage.setItem(ARTIST_KEY, JSON.stringify([...favoriteArtists]));
  }
  ...
  function eventEndDate(event) {
    return new Date(`${event.date}T${addMinutes(event.start, durationMinutes(event))}:00+09:00`);
  }

  function cleanupExpiredFavorites() {
    const now = new Date();
    const validIds = new Set(events.filter(event => eventEndDate(event) > now).map(event => event.id));
    favoriteShows = new Set([...favoriteShows].filter(id => validIds.has(id)));
    saveFavorites();
  }
  ...
  function formatDate(dateString) {
    ...
    return { monthDay: `${m}/${d}`, weekday: "日月火水木金土"[date.getDay()] };
  }
  ...
  function getTopPerformers() {
    const count = new Map();
    events.filter(event => event.ordinary).forEach(event => event.performers.forEach(name => count.set(name, (count.get(name) || 0) + 1)));
    return [...count.entries()].sort((a,b) => b[1] - a[1] || a[0].localeCompare(b[0], "ja")).slice(0, 18).map(([name]) => name);
  }
  ...
  function performerButton(name) {
    return `<button class="performer-button${favoriteArtists.has(name) ? " is-favorite" : ""}" type="button" data-artist="${esc(name)}">${favoriteArtists.has(name) ? "★ " : "☆ "}${esc(name)}</button>`;
  }

  function cardHtml(event) {
    const date = formatDate(event.date);
    const end = addMinutes(event.start, durationMinutes(event));
    const favoriteNames = favoriteArtistNames(event);
    ...
    return `
      <article class="live-card" data-event-id="${esc(event.id)}">
        <button class="show-favorite${favoriteShows.has(event.id) ? " is-favorite" : ""}" type="button" data-show-id="${esc(event.id)}" aria-label="公演をお気に入り登録">${favoriteShows.has(event.id) ? "★" : "☆"}</button>
        ...
        <div class="card-body">
          <div class="card-topline">
            ...
            ${favoriteNames.length ? `<span class="badge favorite-artist-mark">★ お気に入り芸人 ${favoriteNames.length}組</span>` : ""}
          </div>
          ...
          <div class="card-actions"><span class="availability-note">終演は公演形式から算出した予定時刻</span><a class="official-link" href="${esc(event.url)}" target="_blank" rel="noopener">空席・購入を見る →</a></div>
        </div>
      </article>`;
  }

  function renderEventList(container, rows, emptyText) {
    container.innerHTML = rows.length ? rows.map(cardHtml).join("") : `<div class="empty-state"><strong>${esc(emptyText)}</strong></div>`;
  }
  ...
  function renderMainList() {
    ...
    elements.netaCount.textContent = rows.filter(event => event.ordinary).length;
    if (rows[0]) {
      const date = formatDate(rows[0].date);
      elements.nextDate.textContent = date.monthDay;
      elements.nextDateSub.textContent = `${date.weekday}曜 ${rows[0].start}〜`;
    } else {
      elements.nextDate.textContent = "—";
      elements.nextDateSub.textContent = "該当なし";
    }
    elements.resultSummary.textContent = `${state.mode === "neta" ? "ネタ中心" : state.mode === "available" ? "販売中のみ" : "全公演"}：${rows.length}件表示`;
  }

  function renderFavoriteShowsPage() {
    cleanupExpiredFavorites();
    const rows = events.filter(event => favoriteShows.has(event.id) && eventEndDate(event) > new Date()).sort((a,b) => `${a.date}${a.start}`.localeCompare(`${b.date}${b.start}`));
    renderEventList(elements.favShowsList, rows, "お気に入り公演はまだありません");
    elements.showCount.textContent = favoriteShows.size;
  }

  function renderFavoriteArtistsPage() {
    const artists = [...favoriteArtists].sort((a,b) => a.localeCompare(b, "ja"));
    if (!artists.length) {
      elements.favArtistsList.innerHTML = `<div class="empty-state"><strong>お気に入り芸人はまだありません</strong><p>公演カードの出演者名をタップすると登録できます。</p></div>`;
    } else {
      elements.favArtistsList.innerHTML = artists.map(name => {
    ...
    elements.artistCount.textContent = favoriteArtists.size;
  }

  function renderPerformerChips() {
    elements.performerChips.innerHTML = getTopPerformers().map(name => `<button class="performer-chip${state.performer === name ? " is-active" : ""}" type="button" data-filter-artist="${esc(name)}">${esc(name)}</button>`).join("");
  ...
  function updatePage() {
    elements.listPage.hidden = state.page !== "list";
    elements.showsPage.hidden = state.page !== "shows";
    elements.artistsPage.hidden = state.page !== "artists";
  ...
  function resetFilters() {
    ...
    elements.from.value = events[0]?.date || "";
    elements.to.value = "";
    ...
    state.mode = "neta";
    state.performer = "";
    document.querySelectorAll("[data-mode]").forEach(button => button.classList.toggle("is-active", button.dataset.mode === "neta"));
    renderAll();
  }

  function initialize() {
    cleanupExpiredFavorites();
    [...new Set(events.map(event => event.venue))].sort((a,b) => a.localeCompare(b, "ja")).forEach(venue => elements.venue.insertAdjacentHTML("beforeend", `<option>${esc(venue)}</option>`));
    elements.from.value = events[0]?.date || "";
    ...
    [elements.search, elements.venue, elements.genre, elements.from, elements.to, elements.status, elements.price, elements.sort].forEach(control => control.addEventListener("input", renderMainList));
    elements.filterToggle.addEventListener("click", () => elements.advanced.classList.toggle("is-open"));
    elements.reset.addEventListener("click", resetFilters);
    elements.clearPerformer.addEventListener("click", () => { state.performer = ""; renderAll(); });
