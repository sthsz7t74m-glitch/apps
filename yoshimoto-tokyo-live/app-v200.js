(() => {
  ...
  const byId=id=>repo.all().find(e=>e.id===id||e.legacyId===id);
  ...
  function filtered(){
    ...
    return repo.all().filter(e=>{
      ...
      if(venue&&e.venue!==venue)return false;if(genre&&e.genre!==genre)return false;if(from&&e.date<from)return false;if(to&&e.date>to)return false;if(status&&e.status!==status)return false;if(max&&(!e.priceMin||e.priceMin>max))return false;
      return true;
  ...
  function renderShows(){favorites.cleanupExpired(repo.all(),time);const items=repo.all().filter(e=>favorites.isShow(e)&&time.endDate(e)>new Date()).sort((a,b)=>`${a.date}${a.start}`.localeCompare(`${b.date}${b.start}`));renderList($("#favoriteShowsList"),items,"お気に入り公演はまだありません");$("#favoriteShowCount").textContent=items.length;}
  function renderArtists(){const names=[...favorites.artists].sort((a,b)=>a.localeCompare(b,"ja"));const el=$("#favoriteArtistsList");el.innerHTML=names.length?names.map(n=>`<article class="artist-favorite-card"><div><strong>★ ${D.esc(n)}</strong><span>今後の掲載公演 ${repo.all().filter(e=>e.performers.includes(n)&&time.endDate(e)>new Date()).length}件</span></div><button type="button" data-remove-artist="${D.esc(n)}">解除</button></article>`).join(""):`<div class="empty-state"><strong>お気に入り芸人はまだありません</strong></div>`;$("#favoriteArtistCount").textContent=names.length;}
  ...
  function init(){
    ...
    document.addEventListener("click",e=>{
      ...
      const sf=e.target.closest("[data-show-id]");if(sf){const event=byId(sf.dataset.showId);if(event)preserve(()=>favorites.toggleShow(event));return;}
